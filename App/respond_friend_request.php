...
if (isset($_POST['request_id'], $_POST['status'])) {
    ...
    $stmt = $conn->prepare("SELECT origin_member_id, target_member_id FROM cosn.member_relationships WHERE relationship_id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE cosn.member_relationships SET member_relationship_status = ? WHERE relationship_id = ?");
        $stmt->bind_param("si", $status, $request_id);
        $stmt->execute();

        // If the request is accepted, create the reverse relationship (mutual friendship)
        if ($status == 'approved') {
            $stmt = $conn->prepare("INSERT INTO cosn.member_relationships (origin_member_id, target_member_id, member_relationship_type, member_relationship_status) 
